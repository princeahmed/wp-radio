...
function wp_radio_get_stream_data( $post_id )
{
    $stream = [
        ...
        'poster'       => prince_get_meta( $post_id, 'logo', WP_RADIO_ASSETS_URL . '/images/wp-radio-logo.png' ),
...
if ( !function_exists( 'wp_radio_sidebar_layout' ) ) {
 ...
 * @return array
 */
function wp_radio_page_status( $states, $post )
{
    if ( prince_get_option( 'radios_page' ) == $post->ID ) {
        $states[] = __( 'Radios Page', 'wp-radio' );
    }
    return $states;
}
...
add_filter(
    'display_post_states',
    'wp_radio_page_status',
    10,
    2
);
function wp_radio_get_stations_by_country( $country = '' )
{
 ...
 * @return void
 */
function wp_radio_get_template(
    $template_name,
    $args = array(),
    $template_path = 'wp-radio',
    $default_path = ''
)
{
